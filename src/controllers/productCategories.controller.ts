...
export const get = async (limit: number = 10, offset: number = 0): Promise<productCategory[]> => {
  const companies = await db.select().from(productCategoriesTable).limit(limit).offset(offset);
  return companies;
};

export const getOneById = async (id: number): Promise<productCategory | undefined> => {
  const [productCategory] = await db.select().from(productCategoriesTable).where(eq(productCategoriesTable.id, id)).limit(1);
  return productCategory;
};

export const createOne = async (data: productCategoryInsert): Promise<productCategory> => {
  // TODO::validate data
...
export const updateOneById = async (id: number, data: Partial<productCategoryInsert>): Promise<productCategory> => {
  // TODO::validate data
